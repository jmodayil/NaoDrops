...
public class NaoTestObamaMerkel {
  NaoAction na;
  int i;
  int j;
  int mm;
  ...
  public NaoTestObamaMerkel(NaoRobot R, Clock clock) throws IOException {
    ...
    logger.add(R);
    // Initialise Problem
    problem = new ObamaMerkelProblem(this.R, 50.0);
  }
  ...
  public void run(Clock clock) {
    ...
    Action[] act = problem.actions();

    while (!clock.isTerminated()) {
      clock.tick();
      System.out.print("Motion: " + problem.motion + "\n");
      problem.update((ActionArray) act[2]);
    }
    // // Initialise Problem
    // @Monitor
    // SoundEnergyProblem problem = new SoundEnergyProblem(this.R);
    // // Initialise tilecoder
    // TileCodersNoHashing tileCoders = new
    // TileCodersNoHashing(problem.getObservationRanges());
    ...
    // tileCoders.vectorSize());
    // // Set parameters for Sarsa
    // double alpha = .2 / tileCoders.nbActive();
    // double gamma = 0.0;
    // double lambda = .3;
    // // Initialize Sarsa Algorithm:
    // Sarsa sarsa = new Sarsa(alpha, gamma, lambda, toStateAction.vectorSize(),
    // new AMaxTraces()); // Hope
    // // vectorsize
    // // is
    // // correct!
